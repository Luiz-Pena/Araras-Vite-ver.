...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const [[perfil]] = await db.query('SELECT nome, avatar FROM perfis WHERE user_id = ?', [user.id]);
    res.json({ id: user.id, nome: perfil?.nome, avatar: perfil?.avatar });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Erro interno.' });
  }
...
router.get('/me', async (req, res) => {
  ...
  const [[perfil]] = await db.query(
    'SELECT p.user_id AS id, p.nome, p.avatar FROM perfis p WHERE p.user_id = ?',
    [req.session.userId]
  );
